...
export default function LocationModal({ chambre, onClose, onSuccess }) {
  const [clients, setClients] = useState([])
  const [employes, setEmployes] = useState([])
  const [form, setForm] = useState({
    client_id: '', employe_id: '', date_debut: '', date_fin: ''
  })
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:3000/api/employes/hotel/${chambre.hotel_id}`)
      .then(res => setEmployes(res.data))
  }, [])

  async function handleSubmit() {
    setErreur('')
    if (!form.client_id || !form.employe_id || !form.date_debut || !form.date_fin) {
    ...
    setLoading(true)
    try {
      await axios.post('http://localhost:3000/api/locations', {
      ...
      onSuccess('Location créée avec succès !')
      onClose()
    } catch (err) {
      setErreur(err.response?.data?.error || 'Erreur lors de la location.')
    }
    setLoading(false)
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ background: '#FFF8EC', borderRadius: 'var(--radius-sm)', padding: '12px', marginBottom: '20px', border: '0.5px solid var(--or)' }}>
          <p style={{ fontSize: '14px', fontWeight: '600', color: 'var(--or-fonce)' }}>{chambre.hotel_nom}</p>
        ...
        <label style={{ fontSize: '12px', fontWeight: '500', color: 'var(--texte-secondaire)', display: 'block', marginBottom: '6px' }}>Employé responsable</label>
        <select style={inputStyle} value={form.employe_id}
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{
            flex: 1, padding: '11px',
            background: loading ? '#FAC775' : 'var(--or)',
